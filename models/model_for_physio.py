...
class Model(nn.Module):
    def __init__(self, param):
        ...
        n_patches = 1 if self.use_last_patch_only else 4
        ...
        if param.classifier == 'avgpooling_patch_reps':
            ...
            self.classifier = nn.Sequential(
                ...
                nn.Linear(64 * n_patches * 200, param.num_of_classes),
            )
        elif param.classifier == 'all_patch_reps_twolayer':
            self.classifier = nn.Sequential(
                Rearrange('b c s d -> b (c s d)'),
                nn.Linear(64 * n_patches * 200, 200),
                nn.ELU(),
            ...
            self.classifier = nn.Sequential(
                Rearrange('b c s d -> b (c s d)'),
                nn.Linear(64 * n_patches * 200, 4 * 200),
                nn.ELU(),
                nn.Dropout(param.dropout),
                nn.Linear(4 * 200, 200),
                nn.ELU(),
